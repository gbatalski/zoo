...
public class EC2ClusterManager extends ClusterManager {
	...
	public EC2ClusterManager(ZooKeeperClient zkClient) throws IOException {
		super(zkClient);
		ec2Client = new AmazonEC2Client(AWSUtil.getAWSCredentials());
		ec2Client.setEndpoint("ec2.eu-west-1.amazonaws.com");
	}

	public String createEC2Node(String clusterName, String instanceType,
		...
		try {
			...
			for (Instance i : instances) {
				...
				boolean ok = addInstanceNodeToCluster(	clusterName,
														i.getInstanceId());
				if (ok) {
					toReturn = i.getInstanceId();
					logger.info("Successfully created EC2 instance with id "
							+ i.getInstanceId());
					// Poll EC2 instance until it has been created and connected
					// with ZooKeeper.
					...
					while (!(ec2InstanceHasBeenCreated && ec2InstanceHasConnectedToZooKeeper)
							&& timeoutCountdown-- > 0) {
						...
						for (Reservation res : describeInstancesResult.getReservations()) {
							for (Instance resInstance : res.getInstances()) {
								...
								if (resInstance.getInstanceId()
										...
										&& state.getName()
												.equals(EC2_INSTANCE_RUNNING_STATE)) {
									ec2InstanceHasBeenCreated = true;
									addInstanceNodeToCluster(	clusterName,
																i.getInstanceId());
					...
					logger.error("Something went wrong while trying to create zNode with name "
							+ i.getInstanceId());
			}
		} catch (AmazonServiceException ex) {
			// TODO throw exception, saying that zNode creation failed for some
			// reason.
		}
	...
	public void saveDataOnEC2Node(String clusterName, String nodeName,
		...
		saveDataOnInstanceNode(clusterName, nodeName, keypair.getKeyMaterial()
																.getBytes());
	}

	public String createEC2MicroNode(String clusterName, String imageID)
			throws KeeperException, InterruptedException {
		return createEC2Node(	clusterName,
								InstanceType.T1Micro.toString(),
								imageID);
	}

	public String createEC2DefaultMicroNode(String clusterName)
			throws KeeperException, InterruptedException {
		return createEC2Node(	clusterName,
								InstanceType.T1Micro.toString(),
								"ami-359ea941");
