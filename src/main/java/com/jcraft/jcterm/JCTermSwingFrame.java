...
public class JCTermSwingFrame extends JFrame implements ActionListener,
	...
	public JCTermSwingFrame(String name, String userhost) {
		super(name);
		...
		ComponentListener l = new ComponentListener() {
			...
			public void componentResized(ComponentEvent e) {
				...
				int cw = c.getWidth();
				int ch = c.getHeight();
				int cwm = (c.getWidth() - ((JFrame) c).getContentPane()
														.getWidth());
				int chm = (c.getHeight() - ((JFrame) c).getContentPane()
														.getHeight());
				cw -= cwm;
				ch -= chm;
		...
		while (thread != null) {
			...
			try {
				...
				try {
					...
					jschsession = JSchSession.getSession(	user,
															null,
															host,
															port,
															ui,
				...
				if (mode == SHELL) {
					channel = jschsession.getSession()
											.openChannel("shell");
					if (xforwarding) {
						jschsession.getSession()
									.setX11Host(xhost);
						jschsession.getSession()
									.setX11Port(xport + 6000);
				...
				connection = new Connection() {
					...
					public void requestResize(Term term) {
						if (fchannel instanceof ChannelShell) {
							...
							((ChannelShell) fchannel).setPtySize(	c,
																	r,
																	c
																			* term.getCharWidth(),
																	r
	...
	public class MyUserInfo implements UserInfo, UIKeyboardInteractive {
		...
		public boolean promptPassword(String message) {
			Object[] ob = { pword };
			int result = JOptionPane.showConfirmDialog(	JCTermSwingFrame.this.term,
														ob,
														message,
		...
		public String[] promptKeyboardInteractive(String destination,
			...
			if (JOptionPane.showConfirmDialog(	JCTermSwingFrame.this.term,
												panel,
												destination + ": " + name,
												JOptionPane.OK_CANCEL_OPTION,
	...
	public void setProxyHttp(String host, int port) {
		...
		if (proxy_http_host != null && proxy_http_port != 0) {
			proxy = new ProxyHTTP(	proxy_http_host,
									proxy_http_port);
		} else {
			proxy = null;
		}
	...
	public void setProxySOCKS5(String host, int port) {
		...
		if (proxy_socks5_host != null && proxy_socks5_port != 0) {
			proxy = new ProxySOCKS5(proxy_socks5_host,
									proxy_socks5_port);
		} else {
			proxy = null;
		}
	...
	public void setUserHost(String userhost) {
		try {
			...
			if (_host.indexOf(':') != -1) {
				...
				this.host = _host.substring(0, _host.indexOf(':'));
			}
