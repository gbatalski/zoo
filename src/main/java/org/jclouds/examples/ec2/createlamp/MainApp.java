...
public class MainApp {
   ...
   private static void destroySecurityGroupKeyPairAndInstance(EC2Client client, String name) {
      try {
         ...
         RunningInstance instance = findInstanceByKeyName(client, name);
         client.getInstanceServices().terminateInstancesInRegion(null, instance.getId());
   ...
   static RunningInstance runInstance(EC2Client client, String securityGroupName, String keyPairName) {
      String script = new ScriptBuilder() // lamp install script
		       .addStatement(exec("add-apt-repository -y ppa:alestic/ppa"))//
      ...
      Reservation<? extends RunningInstance> reservation = client.getInstanceServices().runInstancesInRegion(null,
               ...
               "ami-bbf539d2", // alestic ami allows auto-invoke of user data scripts
               1, // minimum instances
               1, // maximum instances
               asType(InstanceType.T1_MICRO) // smallest instance size
   ...
   static void blockUntilInstanceTerminated(EC2Client client, RunningInstance instance) throws TimeoutException {
	      // create utilities that wait for the instance to finish
	      RetryablePredicate<RunningInstance> terminatedTester = new RetryablePredicate<RunningInstance>(
	               new InstanceStateTerminated(client), 180, 5, TimeUnit.SECONDS);

	      System.out.printf("%d: %s awaiting instance to terminate %n", System.currentTimeMillis(), instance.getId());
	      if (!terminatedTester.apply(instance))
	         throw new TimeoutException("timeout waiting for instance to terminate: " + instance.getId());
	   }
